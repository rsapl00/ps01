...
public interface CycleChangeRequestRepository extends JpaRepository<CycleChangeRequest, Long> {
    ...
    @Query("SELECT c FROM CycleChangeRequest c WHERE c.divId = :divisionId AND c.expiryTimestamp >= :expiryTs AND c.runDate BETWEEN :startDate AND :endDate ORDER BY c.runDate, c.effectiveDate ASC")
